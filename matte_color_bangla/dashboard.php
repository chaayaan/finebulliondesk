...
<style>
:root {
    /* Core App Design System Palette */
    --navy: #2F4156;
    --teal: #567C8D;
    --sky: #C8D9E6;
    --beige: #F5EFEB;
    ...
    /* Desaturated Status Exception Tokens */
    --success: #3D7A5C;
    --danger: #A6434B;
    --status-paid-bg: #EAF3EE;
    --status-due-bg: #FBECEC;
}
...
/* ---- Page Header (Flat Navy, No Gradient) ---- */
.dash-header {
    background: var(--navy);
    display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap;
    gap: 1rem; padding: 1rem 1.75rem; margin: 0; width: 100%;
    border-radius: 0 0 16px 16px; color: var(--text-on-navy);
}
.dash-header h4 { color: var(--text-on-navy) !important; margin: 0; font-weight: 700; }
.dash-header small { color: rgba(255, 255, 255, 0.78); }

/* ---- Header Period Filter Pills ---- */
.period-pills { display: flex; align-items: center; gap: .4rem; flex-wrap: wrap; }
.period-pill {
    background: rgba(255, 255, 255, 0.14); border: 1px solid rgba(255, 255, 255, 0.3);
    color: #fff; font-weight: 600; font-size: .82rem; border-radius: 8px;
    padding: .32rem .85rem; cursor: pointer; white-space: nowrap; transition: .15s;
}
.period-pill:hover { background: rgba(255, 255, 255, 0.25); }
.period-pill.active { background: #fff; color: var(--navy); border-color: #fff; }
.custom-range-box {
    display: none; align-items: center; gap: .4rem; background: rgba(255, 255, 255, 0.14);
    border: 1px solid rgba(255, 255, 255, 0.3); border-radius: 8px; padding: .25rem .6rem;
}
...
.qa-btn {
    ...
    background: var(--bg-card); border: 1.5px solid var(--border-default); border-radius: 8px;
    padding: .7rem .9rem; text-decoration: none; color: var(--text-primary);
    font-weight: 600; font-size: .88rem; box-shadow: var(--shadow);
    transition: .15s;
}
.qa-btn:hover { border-color: var(--teal); background: var(--bg-hover); color: var(--text-primary); }
.qa-btn .qa-icon {
...
.sc-header {
    ...
    padding: 10px 14px; border-bottom: 1px solid var(--border-default); background: var(--bg-card);
}
...
.sc-header-icon { color: var(--teal); font-size: .85rem; opacity: .8; }
...
.summary-card .sc-title { font-size: .78rem; font-weight: 700; color: var(--text-secondary); text-transform: uppercase; letter-spacing: .04em; }
...
.fin-cell {
    padding: 12px 16px; border-right: 1px solid var(--border-default); border-bottom: 1px solid var(--border-default);
}
.fin-cell:nth-child(3n) { border-right: none; }
.fin-label { font-size: .74rem; color: var(--text-secondary); text-transform: uppercase; letter-spacing: .04em; font-weight: 700; }
.fin-value { font-size: 1.1rem; font-weight: 800; color: var(--text-primary); margin-top: 2px; }
.fin-value.positive { color: var(--success); }
.fin-value.negative { color: var(--danger); }

/* ---- Gold Movement Stat Bar ---- */
.stat-bar { display: grid; grid-template-columns: repeat(4, 1fr); }
.stat-cell {
    padding: 10px 12px; display: flex; align-items: center; gap: 8px;
    border-right: 1px solid var(--border-default); background: var(--bg-card);
}
...
.stat-cell .s-value { font-size: .92rem; font-weight: 700; color: var(--text-primary); }
...
.eb-row { display: flex; flex-direction: column; gap: 4px; padding: 8px 0; border-bottom: 1px solid var(--border-default); }
.eb-row:last-child { border-bottom: none; }
.eb-dot { width: 10px; height: 10px; border-radius: 50%; flex-shrink: 0; }
.eb-name { flex: 1; font-size: .85rem; font-weight: 600; color: var(--text-primary); }
.eb-amount { font-size: .85rem; font-weight: 700; color: var(--navy); }
.eb-bar-track { height: 6px; border-radius: 8px; background: var(--sky); overflow: hidden; }
.eb-bar-fill { height: 100%; border-radius: 8px; }

/* ---- Due List ---- */
.due-row {
    display: flex; align-items: center; justify-content: space-between; gap: 10px;
    padding: 9px 0; border-bottom: 1px solid var(--border-default); text-decoration: none; color: inherit;
}
.due-row:last-child { border-bottom: none; }
.due-row:hover { background: var(--bg-hover); }
.due-type-badge {
    font-size: .68rem; font-weight: 700; padding: 2px 8px; border-radius: 999px;
    text-transform: uppercase; letter-spacing: .02em;
}
.due-type-badge.buy  { background: var(--sky); color: var(--navy); }
.due-type-badge.sale { background: var(--sky); color: var(--navy); }
.due-name { font-size: .85rem; font-weight: 600; color: var(--text-primary); }
.due-amount { font-size: .88rem; font-weight: 800; color: var(--danger); }

/* ---- Recent Transactions ---- */
.recent-row {
    display: flex; align-items: center; gap: 12px; padding: 10px 0; border-bottom: 1px solid var(--border-default);
...
.recent-title { font-size: .87rem; font-weight: 700; color: var(--text-primary); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.recent-sub { font-size: .74rem; color: var(--text-secondary); }
.recent-right { text-align: right; flex-shrink: 0; }
.recent-amount { font-size: .87rem; font-weight: 700; color: var(--text-primary); }
.recent-due { font-size: .7rem; color: var(--danger); font-weight: 600; }
.recent-type-tag {
...
.recent-type-tag.expense { background: var(--status-due-bg); color: var(--danger); }

.view-all-link { font-size: .8rem; font-weight: 700; color: var(--teal); text-decoration: none; }
.view-all-link:hover { color: var(--navy); }

.empty-state { text-align: center; color: var(--text-secondary); padding: 2rem 1rem; font-size: .88rem; }

.skel { background: linear-gradient(90deg, #eaf1f6 25%, #f5efeb 37%, #eaf1f6 63%); background-size: 400% 100%; animation: skel 1.4s ease infinite; border-radius: 8px; }
...
@media (max-width: 991.98px) {
    ...
    .fin-cell:nth-child(3n) { border-right: 1px solid var(--border-default); }
    .fin-cell:nth-child(2n) { border-right: none; }
    .dash-row { grid-template-columns: 1fr; }
}
@media (max-width: 767.98px) {
    .page-inset { padding: 0 1rem 1.5rem; }
    .dash-header { border-radius: 0 0 14px 14px; padding: .85rem 1.1rem; }
    .summary-grid { grid-template-columns: repeat(2, 1fr); gap: .7rem; }
    .stat-bar { grid-template-columns: repeat(2, 1fr); }
    .stat-cell:nth-child(2) { border-right: none; }
    .stat-cell:nth-child(3), .stat-cell:nth-child(4) { border-top: 1px solid var(--border-default); }
...
const SYSTEM_COLORS = ['#2F4156', '#567C8D', '#3D7A5C', '#A6434B', '#8E6C88', '#4A7C59', '#6B5B95', '#D64550'];
...
function renderTrend(trend, label) {
    ...
    const datasets = [
        { label: 'ক্রয়', data: trend.map(t => t.buy), borderColor: SYSTEM_COLORS[0], backgroundColor: SYSTEM_COLORS[0], tension: .3 },
        { label: 'বিক্রয়', data: trend.map(t => t.sale), borderColor: SYSTEM_COLORS[1], backgroundColor: SYSTEM_COLORS[1], tension: .3 },
        { label: 'এক্সচেঞ্জ', data: trend.map(t => t.exchange), borderColor: SYSTEM_COLORS[2], backgroundColor: SYSTEM_COLORS[2], tension: .3 },
        { label: 'খরচ', data: trend.map(t => t.expense), borderColor: SYSTEM_COLORS[3], backgroundColor: SYSTEM_COLORS[3], tension: .3 },
    ];
    ...
    trendChart = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                ...
                y: { beginAtZero: true, ticks: { precision: 0, font: { size: 10 } }, grid: { color: '#C8D9E6', borderDash: [3,3] } },
            },
        },
    });
}
...
function renderBuyVsSale(b) {
    ...
    bvsChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [
                { label: 'ক্রয়', data: [b.buy_amount, b.buy_count], backgroundColor: SYSTEM_COLORS[0], borderRadius: 4, maxBarThickness: 42 },
                { label: 'বিক্রয়', data: [b.sale_amount, b.sale_count], backgroundColor: SYSTEM_COLORS[1], borderRadius: 4, maxBarThickness: 42 },
            ],
        },
        options: {
            responsive: true, maintainAspectRatio: false,
            ...
            scales: {
                ...
                y: { beginAtZero: true, ticks: { display: false }, grid: { color: '#C8D9E6', borderDash: [3,3] } },
            },
        },
    });
}
...
function renderDue(d) {
    ...
    const summaryLine = `
        <div class="d-flex justify-content-between align-items-center mb-2 pb-2" style="border-bottom:1px solid var(--border-default);">
            <span class="fw-semibold small text-muted">মোট বকেয়া লেনদেন</span>
            <span class="fw-bold" style="color:var(--danger);">${d.count} টি</span>
        </div>`;
