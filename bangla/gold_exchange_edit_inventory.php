...
.stock-status-row .ss-label { font-size: 0.83rem; color: var(--muted); }
.stock-status-row .ss-value { font-size: 0.9rem; font-weight: 700; color: var(--status-paid-bg); }
...
.stock-status-row.insufficient .ss-value { color: var(--status-due-bg); }
...
// This exchange's CURRENT final_pure_gold — already deducted from 24K
// inventory. Only the amount ABOVE this (if any) needs to come from
// current stock; editing down FREES stock instead of consuming it.
...
function recalcSummary() {
    ...
    // Stock status — only the INCREASE over this exchange's current
    // final_pure_gold needs to come from live stock; editing down frees
    // stock rather than consuming it, matching the server-side delta rule.
...
// the disabled state applies. The server-side inventory_deduct() check
